  ...
  const handleAddManualCandidate = useCallback(async (text: string, bbox: { x: number, y: number, w: number, h: number }) => {
    ...
    const newCandidate: CandidateWithStatus = {
      ...
      features: {
        ...
        avgTokenLength: text.split(' ').reduce((sum, token) => sum + token.length, 0) / text.split(' ').length,
        hasFrenchWords: FRENCH_DESCRIPTIVE_WORDS.some(word => text.toLowerCase().includes(word)) ? 1 : 0
      },
